...
diag_Xt_A_X(A::Cholesky, X::AbstractMatrix) = diag_At_A(A.U * X)

function diag_Xt_A_Y(X::AM, A::Cholesky, Y::AM)
    @assert size(X) == size(Y)
    return diag_At_B(A.U * X, A.U * Y)
end

diag_Xt_invA_X(A::Cholesky, X::AbstractMatrix) = diag_At_A(A.U' \ X)
