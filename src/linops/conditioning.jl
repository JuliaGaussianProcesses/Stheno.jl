...
function |(g::AbstractGP, c::Observation)
    ...
    return CompositeGP((|, g, C, C \ (c.y - mean(c.f)), c.f), g.gpc)
end

const cond_data = Tuple{typeof(|), AbstractGP, Cholesky, AV{<:Real}, FiniteGP}

function mean_vector((_, g, _, α, fx)::cond_data, x::AV)
    return mean(g(x)) + cov(g(x), fx) * α
end

function cov((_, g, C_ff, _, fx)::cond_data, x::AV)
    return cov(g(x)) - Xt_invA_X(C_ff, cov(fx, g(x)))
end

function cov((_, g, C_ff, _, fx)::cond_data, x::AV, x′::AV)
    return cov(g(x), g(x′)) - Xt_invA_Y(cov(fx, g(x)), C_ff, cov(fx, g(x′)))
end

function cov_diag((_, g, C, _, fx)::cond_data, x::AV)
    return cov_diag(g, x) - diag_Xt_invA_X(C, cov(fx, g(x)))
end

function xcov((_, g, C, _, fx)::cond_data, f′::AbstractGP, x::AV, x′::AV)
    return cov(g(x), f′(x)) - Xt_invA_Y(cov(fx, g(x)), C, cov(fx, f′(x′)))
end

function xcov(f::AbstractGP, (_, g, C, _, fx)::cond_data, x::AV, x′::AV)
    return cov(f(x), g(x′)) - Xt_invA_Y(cov(fx, f(x), C, cov(fx, g(x′))))
end

function xcov_diag((_, g, C, _, fx)::cond_data, f′::AbstractGP, x::AV)
    return xcov_diag(g, f′, x) - diag_Xt_invA_Y(cov(fx, g(x), C, cov(fx, f′(x′))))
end

function xcov_diag(f::AbstractGP, (_, g, C, _, fx)::cond_data, x::AV)
