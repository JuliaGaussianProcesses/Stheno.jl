struct CompositeKernel{Tf, N} <: Kernel
    f::Tf
    x::Tuple{Vararg{Any, N}}
    Composite(f::Tf, x::Vararg{Any, N}) where {Tf, N} = new{Tf, N}(f, x)
end
(c::Composite)(X...) = c.f(map(x->x(X...), c.x)...)
mean(c::Composite, X::AVM) = c.f.(map(x->mean(x, X), c.x)...)
cov(c::Composite, )
