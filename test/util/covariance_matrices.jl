...
using Stheno: Xt_A_X, Xt_A_Y, Xt_invA_Y, Xt_invA_X, diag_AᵀA, diag_AᵀB, diag_Xᵀ_A_X,
    diag_Xᵀ_A_Y, diag_Xᵀ_invA_X, diag_Xᵀ_invA_Y, Xtinv_A_Xinv
...
@testset "cholesky" begin
    ...
    let
        ...
        @test diag_AᵀA(X) ≈ diag(X'X)

        @test diag_AᵀB(X, Z) ≈ diag(X'Z)

        @test diag_Xᵀ_A_X(A, X) ≈ diag(Xt_A_X(A, X))

        @test diag_Xᵀ_A_Y(X, A, Z) ≈ diag(Xt_A_Y(X, A, Z))

        @test diag_Xᵀ_invA_X(A, X) ≈ diag(Xt_invA_X(A, X))

        @test diag_Xᵀ_invA_Y(X, A, Z) ≈ diag(Xt_invA_Y(X, A, Z))
