...
@testset "finite" begin
    ...
    let
        ...
        μ, X, x = ConstantMean(1.5), randn(rng, D, N), RowVector(randn(rng, N))
        μ̂X, μ̂x = FiniteMean(μ, X), FiniteMean(μ, x)

        # Recusion depth 1.
        for (μ̂, X) in zip([μ̂X, μ̂x], [X, x])
            @test size(μ̂, 1) == N
            @test size(μ̂, 2) == 1
            @test length(μ̂) == N
            @test eachindex(μ̂) == RowVector(1:N)
            @test mean(μ̂) == unary_colwise(μ̂, eachindex(μ̂))
            @test mean(μ̂) == unary_colwise(μ, X)

            # Recursion depth 2.
            r = 1:N-1
            μ2 = FiniteMean(μ̂, RowVector(r))
            @test size(μ2, 1) == N - 1
            @test length(μ2) == N - 1
            @test eachindex(μ2) == RowVector(1:N-1)
            @test mean(μ̂)[r] == mean(μ2)
            @test mean(μ̂)[r] == unary_colwise(μ2, eachindex(μ2))
        end

        @test unary_colwise(μ̂X, 1:N-1) == unary_colwise(μ, X[:, 1:N-1])
        @test unary_colwise(μ̂X, 2:N) == unary_colwise(μ, X[:, 2:N])
        @test unary_colwise(μ̂X, 2:N-2) == unary_colwise(μ, X[:, 2:N-2])
