    ...
    @testset "base kernels" begin
        ...
        @testset "RQ" begin
            ...
            @testset "single-input" begin
                ...
                adjoint_test((α, x, x′)->pw(RQ(α), x, x′), Ȳ, 1.5, x0, x2; print_results=true)
                adjoint_test((α, x)->ew(RQ(α), x), ȳ, 1.5, x0; print_results=true)
                adjoint_test((α, x)->pw(RQ(α), x), Ȳ_sq, 1.5, x0; print_results=true)
            end
